...
describe("shouldKeepProduct — raw-only catalog filter", () => {
	...
	it("keeps canned / smoked / cured as-purchased staples", () => {
		expect(shouldKeepProduct(KEEP_CAT, "Tomatoes, red, ripe, canned, packed in tomato juice")).toBe(
			true,
		);
	...
	it("drops less-obvious cooked forms (heated, pan-broil, parboil, blanched, prepared)", () => {
		expect(
			shouldKeepProduct("7", "Pork, cured, ham, slice, separable lean only, heated, pan-broil"),
		).toBe(false);
		expect(shouldKeepProduct("9", "Apples, canned, sweetened, sliced, drained, heated")).toBe(
			false,
		);
	...
	it('keeps "unprepared" items — \\b does not match inside the word', () => {
		expect(shouldKeepProduct(KEEP_CAT, "Potatoes, hash brown, refrigerated, unprepared")).toBe(
			true,
		);
	...
	it('keeps "broilers or fryers" — chicken TYPE, not the cooking verb', () => {
		expect(shouldKeepProduct("5", "Chicken, broilers or fryers, breast, meat only, raw")).toBe(
			true,
		);
		...
		expect(shouldKeepProduct("7", "Ham, slice, separable lean only, heated, pan-broiled")).toBe(
			false,
		);
	});
	...
	it("drops branded products", () => {
		expect(shouldKeepProduct(KEEP_CAT, "Fruit juice smoothie, NAKED JUICE, MIGHTY MANGO")).toBe(
			false,
		);
