<script lang="ts">
	...
	//
	// A keyed `items` mirror is the source of truth (RecipeStep carries no id, so `{#each}` needs
	// stable keys). The persisted projection is a pure `$derived` — NOT an `$effect` write-back
	// into the prop — and the parent reads it on demand at submit via `currentSteps()` (`bind:this`).
	// `steps` is the initial seed only (the host remounts via `{#key}` when the loaded draft changes).
	...
	let items = $state<StepItem[]>(untrack(() => steps.map((s) => stepToItem(s, newKey()))));

	// The persisted projection, derived (never synced via effect). Exposed for the parent's submit.
	const projectedSteps = $derived(items.map(itemToStep));
	export function currentSteps(): RecipeStep[] {
	...
	function addAction() {
		items.push(newStepItem("action", newKey()));
	}
	function addWait() {
		items.push(newStepItem("wait", newKey()));
	}
		...
		{#if item.kind === "wait"}
			...
			>
				...
				<TintedBadge tone="amber">
					<svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
						><path
							d="M5.5 2.5h9a1 1 0 0 1 0 2H14c0 2.5-1.3 3.8-3.2 5.5C12.7 11.7 14 13 14 15.5h.5a1 1 0 0 1 0 2h-9a1 1 0 0 1 0-2H6c0-2.5 1.3-3.8 3.2-5.5C7.3 8.3 6 7 6 4.5h-.5a1 1 0 0 1 0-2Z"
						/></svg
					>
				</TintedBadge>
				<div class="wbody">
					<input
						...
						placeholder={t("recipe.form.waitPlaceholder")}
						aria-label={t("recipe.form.waitPlaceholder")}
					/>
					<span class="wdur">
						<input
							type="number"
							...
							min="1"
							step="1"
							bind:value={item.durationMin}
							aria-label={t("recipe.form.waitDurationLabel")}
						/>
		...
		{:else}
			...
			>
				...
				<div class="sbody">
					<textarea
						bind:value={item.text}
						placeholder={t("recipe.form.stepPlaceholder")}
						aria-label={`${t("recipe.form.stepsTitle")} ${numbers[item.key]}`}
					></textarea>
					<div class="simg">
						{#if item.imageUrl !== null}
							<span class="simg-row">
								<input
									...
									placeholder={t("recipe.form.stepImagePlaceholder")}
									aria-label={t("recipe.form.addStepImage")}
								/>
								<IconButton
									type="button"
									variant="ghost"
						...
						{:else}
							<Button type="button" variant="ghost" size="sm" onclick={() => (item.imageUrl = "")}>
								<svg viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
									><path
										fill-rule="evenodd"
										d="M4 3.5A1.5 1.5 0 0 0 2.5 5v10A1.5 1.5 0 0 0 4 16.5h12A1.5 1.5 0 0 0 17.5 15V5A1.5 1.5 0 0 0 16 3.5H4Zm0 11 3.5-4 2.2 2.6L12.5 9l3.5 4.5H4Z"
										clip-rule="evenodd"
									/><circle cx="7" cy="7.5" r="1.4" /></svg
								>
