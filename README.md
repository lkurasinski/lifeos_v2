...
| `pnpm catalog:import:reset` | …and delete local products absent from the snapshot ⚠️ |
...
| `pnpm railway:publish:reset` | …and delete production products absent from the snapshot ⚠️ |
...
Export before any reset: a product that exists only in a database — one added through the app's
Open Food Facts flow — is otherwise lost. `railway:harvest` recovers such products from
production; there is no equivalent for a local one.
