...
Direction locked 2026-06-04 against the `B3` weekly-plan probe (`context/probes/weekly-plan/`). The earlier blue-accent experiment, the azure-brand idea, and the original cobalt + monospace + brutalist direction are all retired. Two screen references extend it: the product catalog (`context/probes/product-catalog/catalog.html`, master–detail + gauges + grouped profile) and the product add/edit flow (`context/probes/product-catalog/off-add.html`, the editable preview / "AI suggestion surface", shared by manual entry and Open Food Facts import).
...
### Catalog identity palettes (scoped exception)

One deliberate exception to "color earns its place on data scored against target": in the **product catalog**, color is allowed to encode *identity* (which macro, which category), not *score*. Catalog products are reference data, never scored against a target. This is the only place identity color is allowed. Everywhere a value IS scored against the user's target (weekly plan, per-day view, adherence bars), the `positive` / `caution` / `destructive` set still rules and macros stay monochrome graphite.

**Macro identity (gauges).** The four per-product gauges (energy, protein, carbs, fat) each carry a fixed hue so the macros are distinguishable at a glance.
- The **number stays graphite** on its near-solid disc (legibility guard). Color lives in the ring, its gradient sweep and end cap, and the label dot, never on the figure.
...
Both palettes live as reusable tokens in `lifeos-kit.css` (`.g-*`, `.cat-*`). The catalog screen is the locked reference (`context/probes/product-catalog/catalog.html`); category glyphs are placeholder shapes, not final.
...
- **Macro vs target / adherence bar**: a thin rounded (`--radius-pill`) track. Fill is `positive` on or under target, `caution` near the ceiling, `destructive` when over, paired with the value and a sign. The only color on the row.
